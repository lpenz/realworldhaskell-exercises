...
encodeDigits s@(first:rest) =
  ...
  where (left, right) = splitAt 6 rest  -- bugfixed. it had 'splitAt 5' but should be 'splitAt 6'
...
parseRawPPM =
    ...
    parseNat ==> \maxValue ->
    assert (maxValue == 255) "max value out of spec" ==>&   -- TODO <= 255?
    parseByte ==>&
